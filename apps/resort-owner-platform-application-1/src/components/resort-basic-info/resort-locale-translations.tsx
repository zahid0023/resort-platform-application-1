...
type RowWithKey = ResortLocaleRow & { _rkey: string }
...
export function ResortLocaleTranslations({
  ...
  const [rowEditData, setRowEditData] = useState<Record<string, ResortLocaleRow>>({})
  ...
  function startEditRow(key: string, row: ResortLocaleRow) {
  ...
  function patchRowEdit(key: string, patch: Partial<ResortLocaleRow>) {
    setRowEditData((prev) =>
  ...
  async function saveRow(key: string, row: ResortLocaleRow, isNew: boolean) {
  ...
  async function deleteRow(row: ResortLocaleRow) {
    if (!row.id) return
    const key = `e_${row.id}`
    setBusy(key, true)
    try {
  ...
  function addNewLocaleRow() {
    const usedIds = new Set(
      [...form.locales.map((r) => r.locale_id), ...newLocaleRows.map((r) => r.locale_id)].filter(
                ...
                return (
                  <div key={`e_${row.id}`} className="p-4 space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-xs text-muted-foreground">{t("resort.taglineLabel")}</Label>
                        <Input value={row.tagline} disabled className="h-9 text-sm" onChange={() => {}} />
                      </div>
                      <div className="space-y-1.5">
                      ...
                      {row.short_description && (
                        <div className="space-y-1.5">
                          <Label className="text-xs text-muted-foreground">{t("resort.shortDescLabel")}</Label>
                          <Textarea value={row.short_description} disabled rows={2} className="text-sm resize-none" onChange={() => {}} />
                        </div>
                      )}
                      {row.address && (
                        <div className="space-y-1.5">
                          <Label className="text-xs text-muted-foreground">{t("resort.address")}</Label>
