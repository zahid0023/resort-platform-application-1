...
import { facilityPriceTypesService, type FacilityPriceType } from "@/services/facility-price-types"
import { Pagination } from "@/components/shared/pagination"

const PAGE_SIZE = 20

export interface FacilityPriceTypePickerDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  selectedId?: number
  onSelect: (pt: FacilityPriceType) => void
}

export function FacilityPriceTypePickerDialog({
  open,
  onOpenChange,
  selectedId,
  onSelect,
}: FacilityPriceTypePickerDialogProps) {
  const { t } = useTranslation()
  const [priceTypes, setPriceTypes] = useState<FacilityPriceType[]>([])
  ...
  const [detailItem, setDetailItem] = useState<FacilityPriceType | null>(null)
  ...
  async function load(p: number, q: string) {
    setLoading(true)
    try {
      const res = await facilityPriceTypesService.list({
        page: p,
        size: PAGE_SIZE,
        sort_by: "sortOrder",
  ...
  function handleSelect(pt: FacilityPriceType) {
    onSelect(pt)
    onOpenChange(false)
  }
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl p-0 gap-0 overflow-hidden flex flex-col max-h-[85vh]">
          <DialogTitle className="sr-only">{t("resortFacility.selectPriceType")}</DialogTitle>
          ...
          <div className="flex flex-col gap-3 px-5 pt-4 pb-4 pr-12 border-b shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold">{t("resortFacility.selectPriceType")}</p>
                {totalElements > 0 && (
                  <p className="text-xs text-muted-foreground">{totalElements} {t("resortFacility.priceType").toLowerCase()}s</p>
                )}
              </div>
            </div>
        ...
        <Dialog open={!!detailItem} onOpenChange={(o) => { if (!o) setDetailItem(null) }}>
          <DialogContent className="max-w-sm">
            <DialogTitle className="flex items-center gap-2 text-sm font-semibold">
              ...
              {detailItem.locales[0]?.name ?? detailItem.code}
            </DialogTitle>
            <div className="space-y-3 text-sm">
              {detailItem.locales[0]?.description && (
                <p className="text-muted-foreground">{detailItem.locales[0].description}</p>
              )}
              {detailItem.locales[0]?.purpose && (
                <div>
                  <p className="font-medium text-xs uppercase text-muted-foreground mb-1">Purpose</p>
                  <p>{detailItem.locales[0].purpose}</p>
                </div>
              )}
              {detailItem.locales[0]?.usage_example && (
                <div>
                  <p className="font-medium text-xs uppercase text-muted-foreground mb-1">Example</p>
                  <p className="text-muted-foreground italic">{detailItem.locales[0].usage_example}</p>
                </div>
              )}
            </div>
          </DialogContent>
