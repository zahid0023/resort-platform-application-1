...
export interface PriceUnitDetailDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  priceUnit: PriceUnit | null
}

export function PriceUnitDetailDialog({ open, onOpenChange, priceUnit: pu }: PriceUnitDetailDialogProps) {
  if (!pu) return null

  const title = pu.locales[0]?.name ?? pu.code
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg p-0 gap-0 overflow-hidden flex flex-col max-h-[85vh]">
        ...
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 border-b shrink-0 bg-gradient-to-br from-indigo-500/10 via-indigo-500/5 to-transparent">
          <div className="flex size-9 items-center justify-center rounded-xl bg-indigo-500/20 text-indigo-500 shrink-0">
            <Layers className="h-5 w-5" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-bold text-base leading-tight truncate">{title}</p>
            <Badge variant="outline" className="font-mono text-[10px] px-1.5 py-0 h-4 mt-1 border-indigo-500/30 text-indigo-600 dark:text-indigo-400">
              {pu.code}
            </Badge>
          </div>
        </div>
            ...
            <div className="space-y-3">
              {pu.locales.map((loc) => (
                <div key={loc.id} className="rounded-xl border bg-card p-4 space-y-3">
                  ...
                  {loc.calculation_method && (
                    <div>
                      <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide mb-0.5">Calculation Method</p>
                      <p className="text-xs text-muted-foreground leading-relaxed">{loc.calculation_method}</p>
                    </div>
                  )}
