"use client"

import { useEffect, useRef, useState } from "react"
import { Info, Layers, Loader2, Search } from "lucide-react"
import { Badge, Button, Dialog, DialogContent, DialogTitle, Input } from "@resort/shadcn-ui"
import { priceUnitsService, type PriceUnit } from "@/services/price-units"
import { Pagination } from "@/components/shared/pagination"
import { PriceUnitDetailDialog } from "./price-unit-detail-dialog"

const PAGE_SIZE = 20

export interface PriceUnitPickerDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  selectedId?: number
  onSelect: (pu: PriceUnit) => void
}

export function PriceUnitPickerDialog({ open, onOpenChange, selectedId, onSelect }: PriceUnitPickerDialogProps) {
  const [priceUnits, setPriceUnits] = useState<PriceUnit[]>([])
  const [loading, setLoading] = useState(false)
  const [search, setSearch] = useState("")
  const [page, setPage] = useState(0)
  const [totalPages, setTotalPages] = useState(0)
  const [totalElements, setTotalElements] = useState(0)
  const [hasNext, setHasNext] = useState(false)
  const [hasPrevious, setHasPrevious] = useState(false)
  const isFirstSearchRender = useRef(true)
  const [detailPriceUnit, setDetailPriceUnit] = useState<PriceUnit | null>(null)

  useEffect(() => {
    if (open) {
      setSearch("")
      setPage(0)
      isFirstSearchRender.current = true
      load(0, "")
    } else {
      setPriceUnits([])
    }
  }, [open]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (isFirstSearchRender.current) { isFirstSearchRender.current = false; return }
    if (!open) return
    setPage(0)
    const timer = setTimeout(() => load(0, search), 350)
    return () => clearTimeout(timer)
  }, [search]) // eslint-disable-line react-hooks/exhaustive-deps

  async function load(p: number, q: string) {
    setLoading(true)
    try {
      const res = await priceUnitsService.list({
        page: p,
        size: PAGE_SIZE,
        sort_by: "sortOrder",
        code: q.trim() || undefined,
      })
      setPriceUnits(res.data)
      setPage(p)
      setTotalPages(res.total_pages)
      setTotalElements(res.total_elements)
      setHasNext(res.has_next)
      setHasPrevious(res.has_previous)
    } catch {
      // silent
    } finally {
      setLoading(false)
    }
  }

  function handleSelect(pu: PriceUnit) {
    onSelect(pu)
    onOpenChange(false)
  }

  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl p-0 gap-0 overflow-hidden flex flex-col max-h-[85vh]">
          <DialogTitle className="sr-only">Select Price Unit</DialogTitle>

          {/* Header */}
          <div className="flex flex-col gap-3 px-5 pt-4 pb-4 pr-12 border-b shrink-0">
            <div className="flex items-center gap-3">
              <div className="flex size-8 items-center justify-center rounded-lg bg-indigo-500/10 text-indigo-500 shrink-0">
                <Layers className="h-4 w-4" />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold">Select Price Unit</p>
                {totalElements > 0 && (
                  <p className="text-xs text-muted-foreground">{totalElements} price units</p>
                )}
              </div>
            </div>
            <div className="relative">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground pointer-events-none" />
              <Input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search by code…"
                className="pl-8 h-8 text-sm w-full"
              />
            </div>
          </div>

          {/* Body */}
          <div className="flex-1 overflow-y-auto px-5 py-4">
            {loading ? (
              <div className="flex items-center justify-center py-16">
                <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
              </div>
            ) : priceUnits.length === 0 ? (
              <div className="text-center py-16 text-sm text-muted-foreground border rounded-xl border-dashed">
                No price units found.
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {priceUnits.map((pu) => {
                  const isSelected = pu.id === selectedId
                  const name = pu.locales[0]?.name ?? pu.code
                  const description = pu.locales[0]?.description
                  return (
                    <div
                      key={pu.id}
                      className={`relative text-left rounded-xl border transition-all hover:shadow-md hover:-translate-y-0.5 ${
                        isSelected
                          ? "border-indigo-500 bg-indigo-500/5 ring-1 ring-indigo-500/30"
                          : "bg-card hover:border-indigo-500/40"
                      }`}
                    >
                      <button
                        type="button"
                        onClick={() => handleSelect(pu)}
                        className="w-full text-left p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-xl"
                      >
                        <div className="flex items-start gap-3">
                          <div className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 ${
                            isSelected ? "bg-indigo-500/20 text-indigo-500" : "bg-muted text-muted-foreground"
                          }`}>
                            <Layers className="h-4 w-4" />
                          </div>
                          <div className="min-w-0 flex-1 pr-7">
                            <p className="font-semibold text-sm leading-tight truncate">{name}</p>
                            <Badge
                              variant="outline"
                              className={`font-mono text-[10px] px-1.5 py-0 h-4 mt-1 ${
                                isSelected ? "border-indigo-500/30 text-indigo-600 dark:text-indigo-400" : ""
                              }`}
                            >
                              {pu.code}
                            </Badge>
                            {description && (
                              <p className="text-xs text-muted-foreground mt-1.5 line-clamp-2">{description}</p>
                            )}
                          </div>
                        </div>
                      </button>
                      <Button
                        type="button"
                        size="icon"
                        variant="ghost"
                        className="absolute top-2 right-2 h-7 w-7 text-indigo-500 hover:text-indigo-600 hover:bg-indigo-500/10"
                        onClick={(e) => { e.stopPropagation(); setDetailPriceUnit(pu) }}
                        title="View details"
                      >
                        <Info className="h-4 w-4" />
                      </Button>
                    </div>
                  )
                })}
              </div>
            )}
          </div>

          {/* Pagination */}
          {totalPages > 1 && (
            <div className="border-t px-5 py-3 shrink-0">
              <Pagination
                currentPage={page}
                totalPages={totalPages}
                totalElements={totalElements}
                hasNext={hasNext}
                hasPrevious={hasPrevious}
                onPageChange={(p) => load(p, search)}
              />
            </div>
          )}
        </DialogContent>
      </Dialog>

      <PriceUnitDetailDialog
        open={!!detailPriceUnit}
        onOpenChange={(o) => { if (!o) setDetailPriceUnit(null) }}
        priceUnit={detailPriceUnit}
      />
    </>
  )
}
