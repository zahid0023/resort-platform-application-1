...
export function UnitPickerDialog({ open, onOpenChange, selectedId, unitTypeId, onSelect }: UnitPickerDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl p-0 gap-0 overflow-hidden flex flex-col max-h-[85vh]">
        <DialogTitle className="sr-only">Select Unit</DialogTitle>

        {/* Header */}
        <div className="flex flex-col gap-3 px-5 pt-4 pb-4 pr-12 border-b shrink-0">
          <div className="flex items-center gap-3">
            <div className="flex size-8 items-center justify-center rounded-lg bg-teal-500/10 text-teal-500 shrink-0">
              <Ruler className="h-4 w-4" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold">Select Unit</p>
              {totalElements > 0 && (
                <p className="text-xs text-muted-foreground">{totalElements} units</p>
              )}
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {units.map((u) => {
                ...
                const name = u.locales[0]?.name ?? u.code
                const description = u.locales[0]?.description
                return (
                  <button
                    key={u.id}
                    type="button"
                    ...
                    className={`text-left rounded-xl border p-4 transition-all hover:shadow-md hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                      isSelected
                        ...
                        : "bg-card hover:border-teal-500/40"
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      <div className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 text-sm font-bold ${
                        isSelected ? "bg-teal-500/20 text-teal-600 dark:text-teal-400" : "bg-muted text-muted-foreground"
                      }`}>
                        {u.symbol}
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        <Badge
                          ...
                          className={`font-mono text-[10px] px-1.5 py-0 h-4 mt-1 ${
                            isSelected ? "border-teal-500/30 text-teal-600 dark:text-teal-400" : ""
                          }`}
                        >
                          {u.code}
                        </Badge>
