...
interface LocalesContextValue {
   ...
   * time some consumer actually needs the catalog (e.g. a dialog's "Add Translation" picker), via
   * `refresh()`. Session start only loads `totalCount` (see below). */
  locales: Locale[];
   ...
   * (e.g. disabling an "Add Translation" button) without paying for the full catalog fetch.
...
export function LocalesProvider({ children }: { children: React.ReactNode }) {
  ...
  // Collapses concurrent refresh() calls (e.g. two dialogs opening their Translations tab in the
