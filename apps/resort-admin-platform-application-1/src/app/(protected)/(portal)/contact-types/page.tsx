...
export default function ContactTypesPage() {
  ...
  const contactTypeNames = useMemo(
    () => Object.fromEntries(contactTypes.map((c) => [c.id, c.locale?.name ?? ""])),
    [contactTypes],
  );
  ...
  async function openDialog(c: ContactType) {
    try {
      const res = await contactTypesService.get(c.id);
      const full = res.data;
      setMode("view");
      setActiveId(full.id);
      setForm({
        ...
        locale: emptyContactTypeForm.locale,
        // Lazily populated by ContactTypeDialog the first time the Translations tab is selected.
        locales: [],
      });
      setDialogOpen(true);
    } catch (err) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      toast.success(`${t("delete.contactType.title")}: ${deleteTarget.code}`);
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-6">
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("delete.contactType.title")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("delete.contactType.desc", { code: deleteTarget?.code })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
