...
import { PageTypeDialog, emptyPageTypeForm } from "@/components/page-types/page-type-dialog";
import type { PageTypeDialogMode, PageTypeFormState } from "@/components/page-types/types";
import { pageTypesService, type PageType, type ListParams } from "@/services/page-types";
...
export default function PageTypesPage() {
  ...
  const [mode, setMode] = useState<PageTypeDialogMode>("create");
  const [activeId, setActiveId] = useState<number | undefined>(undefined);
  const [form, setForm] = useState<PageTypeFormState>(emptyPageTypeForm);
  ...
  async function refresh(overrides: Partial<ListParams> = {}) {
    setLoading(true);
    try {
      const res = await pageTypesService.list({
        page,
        size: PAGE_SIZE,
        sort_by: sortBy,
        sort_dir: sortDir,
  ...
  const pageTypeNames = useMemo(
    () => Object.fromEntries(pageTypes.map((p) => [p.id, p.locales[0]?.name ?? ""])),
    [pageTypes],
  );

  // Client-side OR filter for "all" field
  const displayPageTypes = useMemo(() => {
    if (searchField !== ALL_FIELD || !search.trim()) return pageTypes;
    const q = search.trim().toLowerCase();
    return pageTypes.filter((p) => {
      const code = p.code.toLowerCase();
      const name = (pageTypeNames[p.id] ?? "").toLowerCase();
      return code.includes(q) || name.includes(q);
    });
  }, [pageTypes, pageTypeNames, search, searchField]);

  function openCreate() {
    setMode("create");
    setActiveId(undefined);
    setForm(emptyPageTypeForm);
    setDialogOpen(true);
  }

  function openDialog(p: PageType) {
    setMode("view");
    setActiveId(p.id);
    setForm({
      code: p.code,
  ...
  async function confirmDelete() {
    ...
    try {
      await pageTypesService.remove(deleteTarget.id);
      toast.success(t("pageType.deleted"));
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-6">
      ...
      <main className="flex flex-col gap-4">
        {loading ? (
          <div className="text-center py-16 text-muted-foreground">{t("pageType.loading")}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {displayPageTypes.map((p) => (
              <PageTypeCard
                key={p.id}
                pageType={p}
                defaultName={pageTypeNames[p.id]}
                onView={openDialog}
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("pageType.deleteTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("pageType.deleteDesc", { code: deleteTarget?.code })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
