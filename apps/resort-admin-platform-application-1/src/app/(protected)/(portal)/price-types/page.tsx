...
export default function PriceTypesPage() {
  ...
  const priceTypeNames = useMemo(
    () => Object.fromEntries(priceTypes.map((p) => [p.id, p.locale?.name ?? ""])),
    [priceTypes],
  );
  ...
  async function openDialog(p: PriceType) {
    try {
      const res = await priceTypesService.get(p.id);
      const full = res.data;
      setMode("view");
      setActiveId(full.id);
      setForm({
        ...
        locale: emptyPriceTypeForm.locale,
        // Lazily populated by PriceTypeDialog the first time the Translations tab is selected.
        locales: [],
        scopes: [],
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      toast.success(`${t("delete.priceType.title")}: ${deleteTarget.code}`);
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-6">
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("delete.priceType.title")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("delete.priceType.desc", { code: deleteTarget?.code })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
