...
import type { ContactTypeDialogMode, ContactTypeFormState, LocaleRow } from "./types";
...
export interface ContactTypeLocaleTranslationsProps {
   ...
   * `GET /contact-types/{id}/locales/count` — not `form.locales`, which only ever holds one page
  ...
  contactTypeLocaleCodes: string[] | null;
}

export function ContactTypeLocaleTranslations({
  mode,
  form,
  onFormChange,
  contactTypeId,
  ...
  availableLocales,
  totalLocaleCount,
  contactTypeLocaleCodes,
}: ContactTypeLocaleTranslationsProps) {
  ...
  // GET /contact-types/{id} and the list endpoint only ever carry the single Accept-Language-matched
  ...
  function refreshLocales(localeCode?: string) {
    ...
    contactTypesService.listLocales(contactTypeId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await contactTypesService.addLocale(contactTypeId, {
        ...
        await contactTypesService.updateLocale(contactTypeId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || contactTypeId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await contactTypesService.removeLocale(contactTypeId, row.id!);
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the contact type's own /locales/count
    ...
    if (contactTypeLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]));
      const existing = contactTypeLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `contactTypeLocaleCodes` would resolve to
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (contactTypeLocaleCodes) return contactTypeLocaleCodes.length + addedCount;
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("placeholder.contactTypeName")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("placeholder.contactTypeDescription")}
                rows={2}
