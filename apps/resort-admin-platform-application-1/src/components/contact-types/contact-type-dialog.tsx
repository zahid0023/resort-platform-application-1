...
import { ArrowRight, Phone, RefreshCw, Save, X } from "lucide-react";
...
import { contactTypesService } from "@/services/contact-types";
...
import { ContactTypeGeneralInfo } from "./contact-type-general-info";
import { ContactTypeLocaleTranslations } from "./contact-type-locale-translations";

export const emptyContactTypeForm: ContactTypeFormState = {
  code: "",
  sort_order: 0,
...
const DRAFT_STORAGE_KEY = "contact-type-dialog-draft";
const DRAFT_SAVE_DEBOUNCE_MS = 500;

function hasDraftContent(f: ContactTypeFormState): boolean {
  return f.code.trim() !== "" || f.locale.name.trim() !== "" || f.locale.description.trim() !== "";
}

function readDraft(): ContactTypeFormState | null {
  try {
    const raw = localStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as ContactTypeFormState;
...
export interface ContactTypeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: ContactTypeDialogMode;
  contactTypeId?: number;
  form: ContactTypeFormState;
  onFormChange: (form: ContactTypeFormState) => void;
  onSaved?: () => void | Promise<void>;
}

export function ContactTypeDialog({
  open,
  onOpenChange,
  mode,
  contactTypeId,
  ...
  // Authoritative set of locale codes this contact type already has a translation for, from
  // `GET /contact-types/{id}/locales/count` — not derived from `form.locales`, which only ever
  ...
  const [contactTypeLocaleCodes, setContactTypeLocaleCodes] = useState<string[] | null>(null);
  ...
  // Owned here so it survives ContactTypeLocaleTranslations-style unmount on tab switch — see
  // [[feedback_tab_content_state]].
  const [localeSearch, setLocaleSearch] = useState("");
  const lastLocaleSearchKey = useRef("");
  const [draftPrompt, setDraftPrompt] = useState<ContactTypeFormState | null>(null);
  ...
  useEffect(() => {
    if (!open) {
      ...
      setContactTypeLocaleCodes(null);
    }
  }, [open]);
  ...
  function validateGeneralInfo(): boolean {
    ...
    if (code.length > CODE_MAX_LENGTH) { toast.error(t("toast.contactTypeCodeMaxLength")); return false; }
    return true;
  }
  ...
  async function fetchLocales(localeCode?: string): Promise<void> {
    ...
    const res = await contactTypesService.listLocales(contactTypeId, { size: 10, localeCode: localeCode || undefined });
    onFormChange({
      ...form,
  ...
  // Companion to fetchLocales — hits the contact type's own /locales/count sub-resource for the
  ...
  async function fetchLocaleCodes(): Promise<void> {
    if (contactTypeId == null) return;
    const res = await contactTypesService.countLocales(contactTypeId);
    setContactTypeLocaleCodes(res.codes);
  }

  // Translations are only fetched once the tab is actually selected — not when the contact type
  ...
  // unfiltered list (see ContactTypeLocaleTranslations' duplicate-locale checks).
  useEffect(() => {
  ...
  // this re-syncs just the shared language count (GET /locales/count) and this contact type's own
  ...
  async function handleRefresh() {
    ...
    try {
      if (activeTab === "general") {
        const res = await contactTypesService.get(contactTypeId);
        const full = res.data;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await contactTypesService.create({
      ...
      clearDraft();
      toast.success(t("contactType.createdToast"));
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
      toast.error((err as Error).message);
  ...
  const headerTitle = mode === "create" ? t("dialog.contactType.new") : (isEditing ? t("dialog.contactType.edit") : t("dialog.contactType.view"));
  const headerDesc = mode === "create" ? t("dialog.contactType.desc.create") : (isEditing ? t("dialog.contactType.desc.edit") : t("dialog.contactType.desc.view"));
      ...
      <Sheet open={open} onOpenChange={(v) => { if (!v) requestClose(); }}>
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            <DialogEntityHeader icon={<Phone className="h-4 w-4" />} title={headerTitle} description={headerDesc} />
            ...
            >
              ...
              <TabsContent value="locales" className="min-h-0 overflow-y-auto px-6 py-5">
                <ContactTypeLocaleTranslations
                  ...
                  contactTypeLocaleCodes={contactTypeLocaleCodes}
                />
              </TabsContent>
            </Tabs>
      ...
      <AlertDialog open={!!draftPrompt} onOpenChange={(v) => { if (!v) discardDraft(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("dialog.restoreDraft.descContactType")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
