...
import type { PriceScopeDialogMode, PriceScopeFormState, LocaleRow } from "./types";
...
export interface PriceScopeLocaleTranslationsProps {
   ...
   * `GET /price-scopes/{id}/locales/count` — not `form.locales`, which only ever holds one page
  ...
  priceScopeLocaleCodes: string[] | null;
}

export function PriceScopeLocaleTranslations({
  mode,
  form,
  onFormChange,
  priceScopeId,
  ...
  availableLocales,
  totalLocaleCount,
  priceScopeLocaleCodes,
}: PriceScopeLocaleTranslationsProps) {
  ...
  // GET /price-scopes/{id} and the list endpoint only ever carry the single Accept-Language-matched
  ...
  function refreshLocales(localeCode?: string) {
    ...
    priceScopesService.listLocales(priceScopeId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await priceScopesService.addLocale(priceScopeId, {
        ...
        await priceScopesService.updateLocale(priceScopeId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || priceScopeId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await priceScopesService.removeLocale(priceScopeId, row.id!);
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the price scope's own /locales/count
    ...
    if (priceScopeLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]));
      const existing = priceScopeLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `priceScopeLocaleCodes` would resolve to
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (priceScopeLocaleCodes) return priceScopeLocaleCodes.length + addedCount;
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("placeholder.priceScopeName")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("placeholder.priceScopeDescription")}
                rows={2}
