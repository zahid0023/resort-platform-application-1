...
import { PriceScopeGeneralInfo } from "./price-scope-general-info";
import { PriceScopeLocaleTranslations } from "./price-scope-locale-translations";

export const emptyPriceScopeForm: PriceScopeFormState = {
  code: "",
  sort_order: 0,
...
const DRAFT_STORAGE_KEY = "price-scope-dialog-draft";
const DRAFT_SAVE_DEBOUNCE_MS = 500;

function hasDraftContent(f: PriceScopeFormState): boolean {
  return f.code.trim() !== "" || f.locale.name.trim() !== "" || f.locale.description.trim() !== "";
}

function readDraft(): PriceScopeFormState | null {
  try {
    const raw = localStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as PriceScopeFormState;
...
export interface PriceScopeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: PriceScopeDialogMode;
  priceScopeId?: number;
  form: PriceScopeFormState;
  onFormChange: (form: PriceScopeFormState) => void;
  onSaved?: () => void | Promise<void>;
}

export function PriceScopeDialog({
  open,
  onOpenChange,
  mode,
  priceScopeId,
  ...
  // Authoritative set of locale codes this price scope already has a translation for, from
  // `GET /price-scopes/{id}/locales/count` — not derived from `form.locales`, which only ever
  ...
  const [priceScopeLocaleCodes, setPriceScopeLocaleCodes] = useState<string[] | null>(null);
  ...
  // Owned here so it survives PriceScopeLocaleTranslations-style unmount on tab switch — see
  // [[feedback_tab_content_state]].
  const [localeSearch, setLocaleSearch] = useState("");
  const lastLocaleSearchKey = useRef("");
  const [draftPrompt, setDraftPrompt] = useState<PriceScopeFormState | null>(null);
  ...
  function validateGeneralInfo(): boolean {
    ...
    if (code.length > CODE_MAX_LENGTH) { toast.error(t("toast.priceScopeCodeMaxLength")); return false; }
    return true;
  }
  ...
  async function fetchLocales(localeCode?: string): Promise<void> {
    ...
    const res = await priceScopesService.listLocales(priceScopeId, { size: 10, localeCode: localeCode || undefined });
    onFormChange({
      ...form,
  ...
  // Companion to fetchLocales — hits the price scope's own /locales/count sub-resource for the
  ...
  async function fetchLocaleCodes(): Promise<void> {
    if (priceScopeId == null) return;
    const res = await priceScopesService.countLocales(priceScopeId);
    setPriceScopeLocaleCodes(res.codes);
  }

  // Translations are only fetched once the tab is actually selected — not when the price scope
  ...
  // unfiltered list (see PriceScopeLocaleTranslations' duplicate-locale checks).
  useEffect(() => {
  ...
  // this re-syncs just the shared language count (GET /locales/count) and this price scope's own
  ...
  async function handleRefresh() {
    ...
    try {
      if (activeTab === "general") {
        const res = await priceScopesService.get(priceScopeId);
        const full = res.data;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await priceScopesService.create({
      ...
      clearDraft();
      toast.success(t("priceScope.createdToast"));
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
      toast.error((err as Error).message);
  ...
  const headerTitle = mode === "create" ? t("dialog.priceScope.new") : (isEditing ? t("dialog.priceScope.edit") : t("dialog.priceScope.view"));
  const headerDesc = mode === "create" ? t("dialog.priceScope.desc.create") : (isEditing ? t("dialog.priceScope.desc.edit") : t("dialog.priceScope.desc.view"));
          ...
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            >
              ...
              <TabsContent value="general" className="min-h-0 overflow-y-auto px-6 py-5">
                <PriceScopeGeneralInfo
                  mode={mode}
                  form={form}
                  onFormChange={(patch) => onFormChange({ ...form, ...patch })}
                  priceScopeId={priceScopeId}
                  onSaved={onSaved}
              ...
              <TabsContent value="locales" className="min-h-0 overflow-y-auto px-6 py-5">
                <PriceScopeLocaleTranslations
                  ...
                  priceScopeLocaleCodes={priceScopeLocaleCodes}
                />
              </TabsContent>
            </Tabs>
      ...
      <AlertDialog open={!!draftPrompt} onOpenChange={(v) => { if (!v) discardDraft(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("dialog.restoreDraft.descPriceScope")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
