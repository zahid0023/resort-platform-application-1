...
import type { UiBlockSectionDialogMode, UiBlockSectionFormState, LocaleRow } from "./types";

type NewLocaleRow = LocaleRow & { _rkey: string };

export interface UiBlockSectionLocaleTranslationsProps {
...
export function UiBlockSectionLocaleTranslations({
  mode,
  form,
  onFormChange,
  sectionId,
  ...
  onEditingChange,
  open,
}: UiBlockSectionLocaleTranslationsProps) {
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await uiBlockSectionsService.addLocale(sectionId, {
        ...
        await uiBlockSectionsService.updateLocale(sectionId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || sectionId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await uiBlockSectionsService.removeLocale(sectionId, row.id);
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder={t("uiBlockSectionDialog.namePlaceholder")}
          disabled={disabled}
          className="h-9 text-sm"
        />
      </div>
