...
export interface ResortPermissionTypeGeneralInfoProps {
  ...
  onFormChange: (patch: Partial<ResortPermissionTypeFormState>) => void;
...
export function ResortPermissionTypeGeneralInfo({
  mode,
  form,
  onFormChange,
  permissionTypeId,
  ...
  onEditingChange,
  open,
}: ResortPermissionTypeGeneralInfoProps) {
  ...
  async function save() {
    ...
    setSubmitting(true);
    try {
      await resortPermissionTypesService.update(permissionTypeId, {
        sort_order: Number(local.sort_order) || 0,
      });
      toast.success(t("resortPermissionType.updatedToast"));
      ...
      <Card>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="rpt-code" className="text-xs font-medium">{t("common.code")} *</Label>
            <Input
              id="rpt-code"
              value={form.code}
          ...
          <div className="space-y-2">
            <Label htmlFor="rpt-sort" className="text-xs font-medium">{t("field.sort")} *</Label>
            <Input
              id="rpt-sort"
              type="number"
              value={sortValue}
