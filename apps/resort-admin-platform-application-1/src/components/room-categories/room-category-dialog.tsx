...
import { ArrowRight, LayoutList, RefreshCw, Save, X } from "lucide-react";
...
import { RoomCategoryGeneralInfo } from "./room-category-general-info";
import { RoomCategoryLocaleTranslations } from "./room-category-locale-translations";
...
const DRAFT_STORAGE_KEY = "room-category-dialog-draft";
const DRAFT_SAVE_DEBOUNCE_MS = 500;

function hasDraftContent(f: RoomCategoryFormState): boolean {
  return f.code.trim() !== "" || f.locale.name.trim() !== "" || f.locale.description.trim() !== "";
}

function readDraft(): RoomCategoryFormState | null {
  try {
    const raw = localStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as RoomCategoryFormState;
...
export interface RoomCategoryDialogProps {
  open: boolean;
...
export function RoomCategoryDialog({
  ...
  // Authoritative set of locale codes this room category already has a translation for, from
  // `GET /room-categories/{id}/locales/count` — not derived from `form.locales`, which only ever
  ...
  const [roomCategoryLocaleCodes, setRoomCategoryLocaleCodes] = useState<string[] | null>(null);
  ...
  // Owned here so it survives RoomCategoryLocaleTranslations-style unmount on tab switch — see
  // [[feedback_tab_content_state]].
  const [localeSearch, setLocaleSearch] = useState("");
  const lastLocaleSearchKey = useRef("");
  const [draftPrompt, setDraftPrompt] = useState<RoomCategoryFormState | null>(null);
  ...
  useEffect(() => {
    if (!open) {
      ...
      setRoomCategoryLocaleCodes(null);
    }
  }, [open]);
  ...
  function validateGeneralInfo(): boolean {
    ...
    if (code.length > CODE_MAX_LENGTH) { toast.error(t("toast.roomCategoryCodeMaxLength")); return false; }
    return true;
  }
  ...
  async function fetchLocales(localeCode?: string): Promise<void> {
    ...
    const res = await roomCategoriesService.listLocales(roomCategoryId, { size: 10, localeCode: localeCode || undefined });
    onFormChange({
      ...form,
  ...
  // Companion to fetchLocales — hits the room category's own /locales/count sub-resource for the
  ...
  async function fetchLocaleCodes(): Promise<void> {
    if (roomCategoryId == null) return;
    const res = await roomCategoriesService.countLocales(roomCategoryId);
    setRoomCategoryLocaleCodes(res.codes);
  }

  // Translations are only fetched once the tab is actually selected — not when the room category
  ...
  // unfiltered list (see RoomCategoryLocaleTranslations' duplicate-locale checks).
  useEffect(() => {
  ...
  // this re-syncs just the shared language count (GET /locales/count) and this room category's own
  ...
  async function handleRefresh() {
    ...
    try {
      if (activeTab === "general") {
        const res = await roomCategoriesService.get(roomCategoryId);
        const full = res.data;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await roomCategoriesService.create({
  ...
  const headerTitle = mode === "create" ? t("roomCategoryDialog.create") : (isEditing ? t("roomCategoryDialog.edit") : t("roomCategoryDialog.view"));
  const headerDesc = mode === "create" ? t("roomCategoryDialog.descCreate") : (isEditing ? t("roomCategoryDialog.descEdit") : t("roomCategoryDialog.descView"));
      ...
      <Sheet open={open} onOpenChange={(v) => { if (!v) requestClose(); }}>
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            <DialogEntityHeader icon={<LayoutList className="h-4 w-4" />} title={headerTitle} description={headerDesc} />
            ...
            >
              ...
              <TabsContent value="locales" className="min-h-0 overflow-y-auto px-6 py-5">
                <RoomCategoryLocaleTranslations
                  mode={mode}
                  form={form}
                  ...
                  roomCategoryLocaleCodes={roomCategoryLocaleCodes}
                />
              </TabsContent>
            </Tabs>
      ...
      <AlertDialog open={!!draftPrompt} onOpenChange={(v) => { if (!v) discardDraft(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("dialog.restoreDraft.descRoomCategory")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
