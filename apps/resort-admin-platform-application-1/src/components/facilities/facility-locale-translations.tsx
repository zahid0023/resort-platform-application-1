...
import type { FacilityDialogMode, FacilityFormState, LocaleRow } from "./types"
...
export interface FacilityLocaleTranslationsProps {
   ...
   * `GET /facilities/{id}/locales/count` — not `form.locales`, which only ever holds one page
  ...
  facilityLocaleCodes: string[] | null
}

export function FacilityLocaleTranslations({
  mode,
  form,
  onFormChange,
  facilityId,
  ...
  totalLocaleCount,
  facilityLocaleCodes,
}: FacilityLocaleTranslationsProps) {
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await addFacilityLocale(facilityId, {
        ...
        await updateFacilityLocale(facilityId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        })
  ...
  async function confirmDelete() {
    ...
    setBusy(key, true)
    try {
      await removeFacilityLocale(facilityId, row.id!)
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the facility's own /locales/count
    ...
    if (facilityLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]))
      const existing = facilityLocaleCodes.map((c) => idByCode.get(c))
  ...
  // in the session) — before that, every code in `facilityLocaleCodes` would resolve to `undefined`
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (facilityLocaleCodes) return facilityLocaleCodes.length + addedCount
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("placeholder.facilityName")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("placeholder.facilityDescription")}
                rows={2}
