...
import type { PriceTypeScopeDialogMode, PriceTypeScopeFormState, LocaleRow } from "./types";
...
export interface PriceTypeScopeLocaleTranslationsProps {
   ...
   * `GET /price-type-scopes/{id}/locales/count` — not `form.locales`, which only ever holds one
   * page (size 10) of the paginated sub-resource and can miss codes past that page. `null` until
   * the first fetch resolves, in which case `usedLocaleIds` falls back to `form.locales`. */
  scopeLocaleCodes: string[] | null;
}

export function PriceTypeScopeLocaleTranslations({
  mode,
  form,
  onFormChange,
  priceTypeScopeId,
  ...
  totalLocaleCount,
  scopeLocaleCodes,
}: PriceTypeScopeLocaleTranslationsProps) {
  ...
  // GET /price-type-scopes/{id} and the list endpoint only ever carry the single
  ...
  function refreshLocales(localeCode?: string) {
    ...
    priceTypeScopesService.listLocales(priceTypeScopeId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await priceTypeScopesService.addLocale(priceTypeScopeId, {
        ...
        await priceTypeScopesService.updateLocale(priceTypeScopeId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || priceTypeScopeId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await priceTypeScopesService.removeLocale(priceTypeScopeId, row.id!);
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the scope's own /locales/count
    ...
    if (scopeLocaleCodes) {
      ...
      const existing = scopeLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `scopeLocaleCodes` would resolve to `undefined`
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (scopeLocaleCodes) return scopeLocaleCodes.length + addedCount;
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("priceTypeScopeDialog.namePlaceholder")}
