...
import { PriceTypeScopeGeneralInfo } from "./price-type-scope-general-info";
import { PriceTypeScopeLocaleTranslations } from "./price-type-scope-locale-translations";

export const emptyPriceTypeScopeForm: PriceTypeScopeFormState = {
  code: "",
  sort_order: 0,
...
const DRAFT_STORAGE_KEY = "price-type-scope-dialog-draft";
const DRAFT_SAVE_DEBOUNCE_MS = 500;

function hasDraftContent(f: PriceTypeScopeFormState): boolean {
  return f.code.trim() !== "" || f.locale.name.trim() !== "" || f.locale.description.trim() !== "";
}

function readDraft(): PriceTypeScopeFormState | null {
  try {
    const raw = localStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as PriceTypeScopeFormState;
...
export interface PriceTypeScopeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: PriceTypeScopeDialogMode;
  priceTypeScopeId?: number;
  form: PriceTypeScopeFormState;
  onFormChange: (form: PriceTypeScopeFormState) => void;
  onSaved?: () => void | Promise<void>;
}

export function PriceTypeScopeDialog({
  open,
  onOpenChange,
  mode,
  priceTypeScopeId,
  ...
  // Authoritative set of locale codes this scope already has a translation for, from
  // `GET /price-type-scopes/{id}/locales/count` — not derived from `form.locales`, which only ever
  ...
  const [scopeLocaleCodes, setScopeLocaleCodes] = useState<string[] | null>(null);
  ...
  // Owned here so it survives PriceTypeScopeLocaleTranslations-style unmount on tab switch — see
  // [[feedback_tab_content_state]].
  const [localeSearch, setLocaleSearch] = useState("");
  const lastLocaleSearchKey = useRef("");
  const [draftPrompt, setDraftPrompt] = useState<PriceTypeScopeFormState | null>(null);
  ...
  function validateGeneralInfo(): boolean {
    ...
    if (code.length > CODE_MAX_LENGTH) { toast.error(t("toast.priceTypeScopeCodeMaxLength")); return false; }
    return true;
  }
  ...
  async function fetchLocales(localeCode?: string): Promise<void> {
    ...
    const res = await priceTypeScopesService.listLocales(priceTypeScopeId, { size: 10, localeCode: localeCode || undefined });
    onFormChange({
      ...form,
  ...
  // Companion to fetchLocales — hits the scope's own /locales/count sub-resource for the
  ...
  async function fetchLocaleCodes(): Promise<void> {
    if (priceTypeScopeId == null) return;
    const res = await priceTypeScopesService.countLocales(priceTypeScopeId);
    setScopeLocaleCodes(res.codes);
  }
  ...
  // unfiltered list (see PriceTypeScopeLocaleTranslations' duplicate-locale checks).
  useEffect(() => {
  ...
  async function handleRefresh() {
    ...
    try {
      if (activeTab === "general") {
        const res = await priceTypeScopesService.get(priceTypeScopeId);
        const full = res.data;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await priceTypeScopesService.create({
      ...
      clearDraft();
      toast.success(t("priceTypeScope.created"));
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
      toast.error((err as Error).message);
  ...
  const headerTitle = mode === "create" ? t("priceTypeScopeDialog.create") : (isEditing ? t("priceTypeScopeDialog.edit") : t("priceTypeScopeDialog.view"));
  const headerDesc = mode === "create" ? t("priceTypeScopeDialog.descCreate") : (isEditing ? t("priceTypeScopeDialog.descEdit") : t("priceTypeScopeDialog.descView"));
          ...
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            >
              ...
              <TabsContent value="general" className="min-h-0 overflow-y-auto px-6 py-5">
                <PriceTypeScopeGeneralInfo
                  mode={mode}
                  form={form}
                  onFormChange={(patch) => onFormChange({ ...form, ...patch })}
                  priceTypeScopeId={priceTypeScopeId}
                  onSaved={onSaved}
              ...
              <TabsContent value="locales" className="min-h-0 overflow-y-auto px-6 py-5">
                <PriceTypeScopeLocaleTranslations
                  mode={mode}
                  form={form}
                  ...
                  scopeLocaleCodes={scopeLocaleCodes}
                />
              </TabsContent>
            </Tabs>
      ...
      <AlertDialog open={!!draftPrompt} onOpenChange={(v) => { if (!v) discardDraft(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("dialog.restoreDraft.descPriceTypeScope")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
