...
interface ApproveUiBlockDialogProps {
  open: boolean;
...
interface SchemaWarnings {
  pageType: string | null;       // unmatched schema.pageType value
  category: string | null;       // unmatched schema.category value
  allowedPages: string[];        // schema.allowedPages values with no DB match
}
...
export function ApproveUiBlockDialog({
  open,
  onOpenChange,
  blockKey,
  blockName,
  ...
  // allowed pages is driven by checkboxes — store selected page type keys
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([pt, cat]) => {
        ...
        // Match schema.pageType → page_type.key (exact — enum values are DB keys)
        ...
        // Match schema.category → ui_block_category.key (exact — enum values are DB keys)
        ...
        // Match schema.allowedPages → page_type.key (exact — enum values are DB keys)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("UI block definition approved and saved!");
      onOpenChange(false);
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Approve UI Block</DialogTitle>
          <DialogDescription>
            Save this UI block definition to the system.
          </DialogDescription>
        </DialogHeader>
        ...
        {loadingData ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <FieldGroup>
              ...
              {/* Allowed Pages — checkboxes from DB page types */}
              <Field>
