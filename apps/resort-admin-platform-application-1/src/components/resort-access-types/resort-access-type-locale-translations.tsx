...
import type { ResortAccessTypeDialogMode, ResortAccessTypeFormState, LocaleRow } from "./types";

type NewLocaleRow = LocaleRow & { _rkey: string };

export interface ResortAccessTypeLocaleTranslationsProps {
  mode: ResortAccessTypeDialogMode;
  form: ResortAccessTypeFormState;
  onFormChange: (form: ResortAccessTypeFormState) => void;
  accessTypeId?: number;
...
export function ResortAccessTypeLocaleTranslations({
  mode,
  form,
  onFormChange,
  accessTypeId,
  ...
  onEditingChange,
  open,
}: ResortAccessTypeLocaleTranslationsProps) {
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await resortAccessTypesService.addLocale(accessTypeId, {
        ...
        await resortAccessTypesService.updateLocale(accessTypeId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || accessTypeId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await resortAccessTypesService.removeLocale(accessTypeId, row.id);
        ...
        {!editing && mode !== "create" && (
          form.locales.length === 0 ? (
            <div className="flex items-center justify-center py-8 text-sm text-muted-foreground">
              ...
              {t("locale.empty.resortAccessType")}
            </div>
          ) : (
                ...
                return (
                  <div key={`e_${row.id}`} className="p-4 space-y-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input value={row.name} disabled placeholder={t("placeholder.resortAccessTypeName")} className="h-9 text-sm" onChange={() => {}} />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground">{t("common.description")}</Label>
                      <Textarea value={row.description} disabled placeholder={t("placeholder.resortAccessTypeDescription")} rows={2} className="text-sm resize-none" onChange={() => {}} />
                    </div>
                  </div>
                );
              })}
                  ...
                  <div key={idx} className="p-4 space-y-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input value={row.name}
                        ...
                        placeholder={t("placeholder.resortAccessTypeName")}
                        className="h-9 text-sm"
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Textarea value={row.description}
                        ...
                        placeholder={t("placeholder.resortAccessTypeDescription")}
                        rows={2}
