...
import { ResortAccessTypeGeneralInfo } from "./resort-access-type-general-info";
import { ResortAccessTypeLocaleTranslations } from "./resort-access-type-locale-translations";

export const emptyResortAccessTypeForm: ResortAccessTypeFormState = {
  code: "",
  sort_order: 0,
  locales: [{ locale_id: "", name: "", description: "", sort_order: 0 }],
};

export interface ResortAccessTypeDialogProps {
  open: boolean;
...
export function ResortAccessTypeDialog({
  open,
  onOpenChange,
  mode,
  accessTypeId,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await resortAccessTypesService.create({
        code,
      ...
      toast.success(t("resortAccessType.createdToast"));
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
  ...
  const headerTitle = mode === "create"
    ? t("resortAccessTypeDialog.new")
    : (isEditing ? t("resortAccessTypeDialog.edit") : t("resortAccessTypeDialog.view"));
  const headerDesc = mode === "create"
    ? t("resortAccessTypeDialog.desc.create")
    : (isEditing ? t("resortAccessTypeDialog.desc.edit") : t("resortAccessTypeDialog.desc.view"));
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) requestClose(); }}>
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            <DialogEntityHeader icon={<KeyRound className="h-4 w-4" />} title={headerTitle} description={headerDesc} />

            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              <ResortAccessTypeGeneralInfo
                mode={mode}
                form={form}
                ...
                open={open}
              />
              <ResortAccessTypeLocaleTranslations
                mode={mode}
                form={form}
