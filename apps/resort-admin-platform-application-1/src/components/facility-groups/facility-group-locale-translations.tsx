...
import type { FacilityGroupDialogMode, FacilityGroupFormState, LocaleRow } from "./types"
...
export interface FacilityGroupLocaleTranslationsProps {
   ...
   * `GET /facility-groups/{id}/locales/count` — not `form.locales`, which only ever holds one page
  ...
  facilityGroupLocaleCodes: string[] | null
}

export function FacilityGroupLocaleTranslations({
  mode,
  form,
  onFormChange,
  facilityGroupId,
  ...
  availableLocales,
  totalLocaleCount,
  facilityGroupLocaleCodes,
}: FacilityGroupLocaleTranslationsProps) {
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || facilityGroupId == null || !pendingDeleteRow.id) return
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the group's own /locales/count
    ...
    if (facilityGroupLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]))
      const existing = facilityGroupLocaleCodes.map((c) => idByCode.get(c))
  ...
  // in the session) — before that, every code in `facilityGroupLocaleCodes` would resolve to
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (facilityGroupLocaleCodes) return facilityGroupLocaleCodes.length + addedCount
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("placeholder.facilityGroupName")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("placeholder.facilityGroupDescription")}
                rows={2}
