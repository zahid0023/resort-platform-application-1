...
export interface FacilityScopeCardProps {
  ...
  onDelete?: (scope: FacilityScope) => void;
}

export function FacilityScopeCard({ scope, defaultName, onView, onDelete }: FacilityScopeCardProps) {
  ...
  return (
    <Card
      ...
      onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onView?.(scope); } }}
      ...
      <CardHeader>
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate font-mono">{scope.code}</p>
          </div>
        </div>
        {(onView || onDelete) && (
          <CardAction
