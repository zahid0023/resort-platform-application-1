...
import type { FacilityScopeDialogMode, FacilityScopeFormState, LocaleRow } from "./types";
...
export interface FacilityScopeLocaleTranslationsProps {
...
export function FacilityScopeLocaleTranslations({
  ...
  // GET /facility-scopes/{id} and the list endpoint only ever carry the single Accept-Language-matched
  ...
  function refreshLocales(localeCode?: string) {
    ...
    facilityScopesService.listLocales(facilityScopeId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await facilityScopesService.addLocale(facilityScopeId, {
        ...
        await facilityScopesService.updateLocale(facilityScopeId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || facilityScopeId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await facilityScopesService.removeLocale(facilityScopeId, row.id!);
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("placeholder.facilityScopeName")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("placeholder.facilityScopeDescription")}
                rows={2}
