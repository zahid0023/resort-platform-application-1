...
import { ArrowRight, BedDouble, RefreshCw, Save, X } from "lucide-react";
...
import { BedTypeGeneralInfo } from "./bed-type-general-info";
import { BedTypeLocaleTranslations } from "./bed-type-locale-translations";

export const emptyBedTypeForm: BedTypeFormState = {
  code: "",
  sort_order: 0,
...
const DRAFT_STORAGE_KEY = "bed-type-dialog-draft";
const DRAFT_SAVE_DEBOUNCE_MS = 500;

function hasDraftContent(f: BedTypeFormState): boolean {
  return f.code.trim() !== "" || f.locale.name.trim() !== "" || f.locale.description.trim() !== "";
}

function readDraft(): BedTypeFormState | null {
  try {
    const raw = localStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as BedTypeFormState;
...
export interface BedTypeDialogProps {
  open: boolean;
...
export function BedTypeDialog({
  ...
  // `GET /bed-types/{id}/locales/count` — not derived from `form.locales`, which only ever holds one
  // page (size 10) of the paginated sub-resource and can undercount past that. Compared against the
  // platform-wide codes from `useLocales()` to know which languages are still addable.
  const [bedTypeLocaleCodes, setBedTypeLocaleCodes] = useState<string[] | null>(null);
  ...
  // the current session. The manual Refresh button only needs `refreshLocalesCount`.
  ...
  // Owned here so it survives BedTypeLocaleTranslations-style unmount on tab switch.
  const [localeSearch, setLocaleSearch] = useState("");
  const lastLocaleSearchKey = useRef("");
  const [draftPrompt, setDraftPrompt] = useState<BedTypeFormState | null>(null);
  ...
  function validateGeneralInfo(): boolean {
    ...
    if (code.length > CODE_MAX_LENGTH) { toast.error(t("toast.bedTypeCodeMaxLength")); return false; }
    return true;
  }
  ...
  async function fetchLocales(localeCode?: string): Promise<void> {
    ...
    const res = await bedTypesService.listLocales(bedTypeId, { size: 10, localeCode: localeCode || undefined });
    onFormChange({
      ...form,
  ...
  // Companion to fetchLocales — hits the bed type's own /locales/count sub-resource for the
  ...
  async function fetchLocaleCodes(): Promise<void> {
    if (bedTypeId == null) return;
    const res = await bedTypesService.countLocales(bedTypeId);
    setBedTypeLocaleCodes(res.codes);
  }

  // Translations are only fetched once the tab is actually selected — not when the bed type card
  ...
  // unfiltered list (see BedTypeLocaleTranslations' duplicate-locale checks).
  useEffect(() => {
  ...
  // this re-syncs just the shared language count (GET /locales/count) and this bed type's own
  ...
  async function handleRefresh() {
    ...
    try {
      if (activeTab === "general") {
        const res = await bedTypesService.get(bedTypeId);
        const full = res.data;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      clearDraft();
      toast.success(t("bedType.createdToast"));
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
      toast.error((err as Error).message);
  ...
  const headerTitle = mode === "create" ? t("dialog.bedType.new") : (isEditing ? t("dialog.bedType.edit") : t("dialog.bedType.view"));
  const headerDesc = mode === "create" ? t("dialog.bedType.desc.create") : (isEditing ? t("dialog.bedType.desc.edit") : t("dialog.bedType.desc.view"));
      ...
      <Sheet open={open} onOpenChange={(v) => { if (!v) requestClose(); }}>
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            <DialogEntityHeader icon={<BedDouble className="h-4 w-4" />} title={headerTitle} description={headerDesc} />
            ...
            >
              ...
              <TabsContent value="locales" className="min-h-0 overflow-y-auto px-6 py-5">
                <BedTypeLocaleTranslations
                  ...
                  bedTypeLocaleCodes={bedTypeLocaleCodes}
                />
              </TabsContent>
            </Tabs>
      ...
      <AlertDialog open={!!draftPrompt} onOpenChange={(v) => { if (!v) discardDraft(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("dialog.restoreDraft.descBedType")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
