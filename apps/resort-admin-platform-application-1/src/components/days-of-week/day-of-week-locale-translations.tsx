...
import type { DayOfWeekFormState, LocaleRow } from "./types";

type NewLocaleRow = LocaleRow & { _rkey: string };

export interface DayOfWeekLocaleTranslationsProps {
   ...
   * `GET /days-of-week/{id}/locales/count` — not `form.locales`, which only ever holds one page
  ...
  dayOfWeekLocaleCodes: string[] | null;
}

export function DayOfWeekLocaleTranslations({
  form,
  onFormChange,
  dayOfWeekId,
  onSaved,
  ...
  availableLocales,
  totalLocaleCount,
  dayOfWeekLocaleCodes,
}: DayOfWeekLocaleTranslationsProps) {
  ...
  // GET /days-of-week/{id} and the list endpoint only ever carry the single Accept-Language-matched
  ...
  function refreshLocales(localeCode?: string) {
    ...
    daysOfWeekService.listLocales(dayOfWeekId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    if (!data.short_name.trim()) { toast.error(t("toast.localeShortNameRequired", { n: 1 })); return; }
    setBusy(key, true);
    try {
      if (isNew) {
        await daysOfWeekService.addLocale(dayOfWeekId, {
        ...
        await daysOfWeekService.updateLocale(dayOfWeekId, row.id!, {
          name: data.name.trim(),
          short_name: data.short_name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || dayOfWeekId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await daysOfWeekService.removeLocale(dayOfWeekId, row.id!);
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the day's own /locales/count
    ...
    if (dayOfWeekLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]));
      const existing = dayOfWeekLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `dayOfWeekLocaleCodes` would resolve to
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (dayOfWeekLocaleCodes) return dayOfWeekLocaleCodes.length + addedCount;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Search is hidden while a row is open — editing always needs the complete, unfiltered list. */}
      {!editing && (form.locales.length > 0 || search.trim() !== "") && (
      ...
      <Card className="gap-0 py-0 overflow-hidden">
        {/* Each translation is edited independently via its own pencil icon — no section-wide edit
                ...
                <div key={key} className="p-4 space-y-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      placeholder={t("placeholder.dayOfWeekName")}
                      disabled={!rowEditing}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      placeholder={t("placeholder.dayOfWeekShortName")}
                      disabled={!rowEditing}
                      className="h-9 text-sm"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea
                      ...
                      placeholder={t("placeholder.dayOfWeekDescription")}
                      disabled={!rowEditing}
                      rows={2}
