...
import type { CommunicationChannelDialogMode, CommunicationChannelFormState, LocaleRow } from "./types";
...
export interface CommunicationChannelLocaleTranslationsProps {
   ...
   * `GET /communication-channels/{id}/locales/count` — not `form.locales`, which only ever holds
   * one page (size 10) of the paginated sub-resource and can miss codes past that page. `null`
   * until the first fetch resolves, in which case `usedLocaleIds` falls back to `form.locales`. */
  channelLocaleCodes: string[] | null;
}

export function CommunicationChannelLocaleTranslations({
  mode,
  form,
  onFormChange,
  channelId,
  ...
  totalLocaleCount,
  channelLocaleCodes,
}: CommunicationChannelLocaleTranslationsProps) {
  ...
  // GET /communication-channels/{id} and the list endpoint only ever carry the single
  ...
  function refreshLocales(localeCode?: string) {
    ...
    communicationChannelsService.listLocales(channelId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await communicationChannelsService.addLocale(channelId, {
        ...
        await communicationChannelsService.updateLocale(channelId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || channelId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await communicationChannelsService.removeLocale(channelId, row.id!);
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the channel's own /locales/count
    ...
    if (channelLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]));
      const existing = channelLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `channelLocaleCodes` would resolve to `undefined`
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (channelLocaleCodes) return channelLocaleCodes.length + addedCount;
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("commChannelDialog.namePlaceholder")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("commChannelDialog.descriptionPlaceholder")}
                rows={2}
