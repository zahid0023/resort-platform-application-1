...
import { ArrowRight, Radio, RefreshCw, Save, X } from "lucide-react";
...
const DRAFT_STORAGE_KEY = "communication-channel-dialog-draft";
const DRAFT_SAVE_DEBOUNCE_MS = 500;

function hasDraftContent(f: CommunicationChannelFormState): boolean {
  return f.code.trim() !== "" || f.locale.name.trim() !== "" || f.locale.description.trim() !== "";
}

function readDraft(): CommunicationChannelFormState | null {
  try {
    const raw = localStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as CommunicationChannelFormState;
...
export function CommunicationChannelDialog({
  ...
  // Authoritative set of locale codes this channel already has a translation for, from
  // `GET /communication-channels/{id}/locales/count` — not derived from `form.locales`, which only
  // ever holds one page (size 10) of the paginated sub-resource and can undercount past that.
  // Compared against the platform-wide codes from `useLocales()` to know which languages are still
  // addable. `null` until the first fetch resolves.
  const [channelLocaleCodes, setChannelLocaleCodes] = useState<string[] | null>(null);
  // The language catalog is loaded once per session by LocalesProvider and shared across every
  // entity dialog — no per-dialog fetch needed. `refreshLocalesCatalog` is only used as an explicit
  // re-sync (a fallback in prepareAddLocale) for locales created elsewhere during the current
  // session. The manual Refresh button only needs `refreshLocalesCount` (GET /locales/count) — the
  // full paginated catalog (GET /locales) is only ever needed by the +Add language picker.
  ...
  // Owned here so it survives CommunicationChannelLocaleTranslations-style unmount on tab switch —
  // see [[feedback_tab_content_state]].
  const [localeSearch, setLocaleSearch] = useState("");
  const lastLocaleSearchKey = useRef("");
  const [draftPrompt, setDraftPrompt] = useState<CommunicationChannelFormState | null>(null);
  ...
  function isGeneralInfoValid(): boolean {
    ...
    return code !== "" && code.length <= CODE_MAX_LENGTH && (form.is_phone || form.is_email || form.is_url);
  }
  ...
  function validateGeneralInfo(): boolean {
    ...
    if (code.length > CODE_MAX_LENGTH) { toast.error(t("toast.commChannelCodeMaxLength")); return false; }
    if (!form.is_phone && !form.is_email && !form.is_url) { toast.error(t("toast.commChannelTypeRequired")); return false; }
    return true;
  }
  ...
  async function fetchLocales(localeCode?: string): Promise<void> {
    ...
    const res = await communicationChannelsService.listLocales(channelId, { size: 10, localeCode: localeCode || undefined });
    onFormChange({
      ...form,
  ...
  // Companion to fetchLocales — hits the channel's own /locales/count sub-resource for the
  ...
  async function fetchLocaleCodes(): Promise<void> {
    if (channelId == null) return;
    const res = await communicationChannelsService.countLocales(channelId);
    setChannelLocaleCodes(res.codes);
  }

  // Translations are only fetched once the tab is actually selected — not when the channel card is
  ...
  // unfiltered list (see CommunicationChannelLocaleTranslations' duplicate-locale checks).
  useEffect(() => {
  ...
  // this re-syncs just the shared language count (GET /locales/count) and this channel's own
  ...
  async function handleRefresh() {
    ...
    try {
      if (activeTab === "general") {
        const res = await communicationChannelsService.get(channelId);
        const full = res.data;
        onFormChange({
          ...form,
  ...
  const headerTitle = mode === "create" ? t("commChannelDialog.create") : (isEditing ? t("commChannelDialog.edit") : t("commChannelDialog.view"));
  const headerDesc = mode === "create" ? t("commChannelDialog.descCreate") : (isEditing ? t("commChannelDialog.descEdit") : t("commChannelDialog.descView"));
            ...
            >
              ...
              <TabsContent value="locales" className="min-h-0 overflow-y-auto px-6 py-5">
                <CommunicationChannelLocaleTranslations
                  ...
                  channelLocaleCodes={channelLocaleCodes}
                />
              </TabsContent>
            </Tabs>
      ...
      <AlertDialog open={!!draftPrompt} onOpenChange={(v) => { if (!v) discardDraft(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("dialog.restoreDraft.descCommChannel")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
