...
interface PageTypeDialogProps {
  ...
  editing: PageType | null
  onSuccess: () => void
}

const empty = { key: "", name: "", description: "" }

export function PageTypeDialog({ open, onOpenChange, editing, onSuccess }: PageTypeDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await updatePageType(editing.id, payload)
        toast.success("Page type updated successfully!")
      } else {
        await createPageType(payload)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{editing ? "Edit Page Type" : "Create Page Type"}</DialogTitle>
          <DialogDescription>
            {editing
              ? "Update the details of this page type."
              : "Add a new page type to the system."}
          </DialogDescription>
        </DialogHeader>
          ...
          <FieldGroup>
            <Field>
              ...
              <Input
                ...
                required
              />
              <FieldDescription>Unique identifier key (e.g. LANDING, ROOMS).</FieldDescription>
            </Field>
            ...
            <Field>
              ...
              />
              <FieldDescription>Optional description for this page type.</FieldDescription>
            </Field>
          </FieldGroup>
