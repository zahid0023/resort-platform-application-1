...
import { pageTypesService } from "@/services/page-types";
import type { Locale } from "@/services/locales";
import { toast } from "sonner";
import type { PageTypeDialogMode, PageTypeFormState } from "./types";
import { PageTypeGeneralInfo } from "./page-type-general-info";
import { PageTypeLocaleTranslations } from "./page-type-locale-translations";

export const emptyPageTypeForm: PageTypeFormState = {
  code: "",
  sort_order: 0,
  locales: [{ locale_id: "", name: "", description: "", sort_order: 0 }],
};

export interface PageTypeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: PageTypeDialogMode;
  pageTypeId?: number;
  form: PageTypeFormState;
  onFormChange: (form: PageTypeFormState) => void;
...
export function PageTypeDialog({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success(t("pageType.created"));
      onOpenChange(false);
      await onSaved?.();
    } catch (err) {
  ...
  const headerTitle = mode === "create"
    ? t("pageTypeDialog.create")
    : (isEditing ? t("pageTypeDialog.edit") : t("pageTypeDialog.view"));
  const headerDesc = mode === "create"
    ? t("pageTypeDialog.descCreate")
    : (isEditing ? t("pageTypeDialog.descEdit") : t("pageTypeDialog.descView"));
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) requestClose(); }}>
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
            ...
            <DialogEntityHeader icon={<FileText className="h-4 w-4" />} title={headerTitle} description={headerDesc} />
