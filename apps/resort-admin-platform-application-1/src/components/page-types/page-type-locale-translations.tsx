...
import type { PageTypeDialogMode, PageTypeFormState, LocaleRow } from "./types";

type NewLocaleRow = LocaleRow & { _rkey: string };

export interface PageTypeLocaleTranslationsProps {
...
export function PageTypeLocaleTranslations({
  mode,
  form,
  onFormChange,
  pageTypeId,
  ...
  onEditingChange,
  open,
}: PageTypeLocaleTranslationsProps) {
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await pageTypesService.addLocale(pageTypeId, {
        ...
        await pageTypesService.updateLocale(pageTypeId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || pageTypeId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await pageTypesService.removeLocale(pageTypeId, row.id);
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder={t("pageTypeDialog.namePlaceholder")}
          disabled={disabled}
          className="h-9 text-sm"
        />
      </div>
