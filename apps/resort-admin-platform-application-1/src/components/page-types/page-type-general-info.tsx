...
export interface PageTypeGeneralInfoProps {
  mode: PageTypeDialogMode;
  form: PageTypeFormState;
  onFormChange: (patch: Partial<PageTypeFormState>) => void;
  pageTypeId?: number;
...
export function PageTypeGeneralInfo({
  mode,
  form,
  onFormChange,
  pageTypeId,
  ...
  async function save() {
    ...
    setSubmitting(true);
    try {
      await pageTypesService.update(pageTypeId, {
        sort_order: Number(local.sort_order) || 0,
      });
      toast.success(t("pageType.updated"));
      ...
      <Card>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="pt-code" className="text-xs font-medium">{t("common.code")} *</Label>
            <Input
              id="pt-code"
              value={form.code}
          ...
          <div className="space-y-2">
            <Label htmlFor="pt-sort" className="text-xs font-medium">{t("field.sort")} *</Label>
            <Input
              id="pt-sort"
              type="number"
              value={sortValue}
