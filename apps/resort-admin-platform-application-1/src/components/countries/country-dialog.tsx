...
export function CountryDialog({
  ...
  // Authoritative set of locale codes this country already has a translation for, from
  // `GET /countries/{id}/locales/count` — not derived from `form.locales`, which only ever holds
  ...
  const [countryLocaleCodes, setCountryLocaleCodes] = useState<string[] | null>(null);
  ...
  // Companion to fetchLocales — hits the country's own /locales/count sub-resource for the
  ...
  async function fetchLocaleCodes(): Promise<void> {
    if (countryId == null) return;
    const res = await countriesService.countLocales(countryId);
    setCountryLocaleCodes(res.codes);
  }
  ...
  // language count (GET /locales/count) — the full paginated catalog (GET /locales) is only ever
  // needed by the +Add language picker, not by a refresh.
            ...
            >
              ...
              <TabsContent value="locales" className="min-h-0 overflow-y-auto px-6 py-5">
                <CountryLocaleTranslations
                  ...
                  countryLocaleCodes={countryLocaleCodes}
                />
              </TabsContent>
            </Tabs>
