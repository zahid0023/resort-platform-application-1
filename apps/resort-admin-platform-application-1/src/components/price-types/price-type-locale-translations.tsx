...
import type { PriceTypeDialogMode, PriceTypeFormState, LocaleRow } from "./types";
...
export interface PriceTypeLocaleTranslationsProps {
   ...
   * `GET /price-types/{id}/locales/count` — not `form.locales`, which only ever holds one page
  ...
  priceTypeLocaleCodes: string[] | null;
}

export function PriceTypeLocaleTranslations({
  mode,
  form,
  onFormChange,
  priceTypeId,
  ...
  availableLocales,
  totalLocaleCount,
  priceTypeLocaleCodes,
}: PriceTypeLocaleTranslationsProps) {
  ...
  // GET /price-types/{id} and the list endpoint only ever carry the single Accept-Language-matched
  ...
  function refreshLocales(localeCode?: string) {
    ...
    priceTypesService.listLocales(priceTypeId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  async function saveRow(key: string, row: LocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await priceTypesService.addLocale(priceTypeId, {
        ...
        await priceTypesService.updateLocale(priceTypeId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
          purpose: data.purpose?.trim() || undefined,
  ...
  async function confirmDelete() {
    if (!pendingDeleteRow || priceTypeId == null || !pendingDeleteRow.id) return;
    const row = pendingDeleteRow;
    const key = rowKey(row);
    setPendingDeleteRow(null);
    setBusy(key, true);
    try {
      await priceTypesService.removeLocale(priceTypeId, row.id!);
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the price type's own /locales/count
    ...
    if (priceTypeLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]));
      const existing = priceTypeLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `priceTypeLocaleCodes` would resolve to
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (priceTypeLocaleCodes) return priceTypeLocaleCodes.length + addedCount;
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("placeholder.priceTypeName")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("placeholder.priceTypeDescription")}
                rows={2}
                className="text-sm resize-none"
              />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground">{t("field.purpose")} *</Label>
              <Textarea value={form.locale.purpose}
                onChange={(e) => onFormChange({ ...form, locale: { ...form.locale, purpose: e.target.value.replace(NON_ASCII, "") } })}
                placeholder={t("placeholder.priceTypePurpose")}
                rows={2}
                className="text-sm resize-none"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.usage_example}
                onChange={(e) => onFormChange({ ...form, locale: { ...form.locale, usage_example: e.target.value.replace(NON_ASCII, "") } })}
                placeholder={t("placeholder.priceTypeUsageExample")}
                rows={2}
                className="text-sm resize-none"
              />
            </div>
                  ...
                  <div key={key} className="p-4 space-y-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        ...
                        onChange={(e) => patchRowEdit(key, { purpose: e.target.value })}
                        placeholder={t("placeholder.priceTypePurpose")}
                        disabled={!rowEditing}
                        rows={2}
                        className="text-sm resize-none"
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs text-muted-foreground">{t("field.usageExample")}</Label>
                      <Textarea
                        value={editData.usage_example}
                        onChange={(e) => patchRowEdit(key, { usage_example: e.target.value })}
                        placeholder={t("placeholder.priceTypeUsageExample")}
                        disabled={!rowEditing}
                        rows={2}
                        className="text-sm resize-none"
