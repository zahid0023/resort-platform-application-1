...
export interface PriceUnitLocaleTranslationsProps {
   ...
   * `GET /price-units/{id}/locales/count` — not `form.locales`, which only ever holds one page
  ...
  priceUnitLocaleCodes: string[] | null;
}
...
export function PriceUnitLocaleTranslations({
  ...
  // GET /price-units/{id} and the list endpoint only ever carry the single Accept-Language-matched
  ...
  function refreshLocales(localeCode?: string) {
    ...
    priceUnitsService.listLocales(priceUnitId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the price unit's own /locales/count
    ...
    if (priceUnitLocaleCodes) {
      const idByCode = new Map(availableLocales.map((l) => [l.code, l.id]));
      const existing = priceUnitLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `priceUnitLocaleCodes` would resolve to
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (priceUnitLocaleCodes) return priceUnitLocaleCodes.length + addedCount;
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.locale.name}
                ...
                placeholder={t("placeholder.priceUnitName")}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.description}
                ...
                placeholder={t("placeholder.priceUnitDescription")}
                rows={2}
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.purpose}
                ...
                placeholder={t("placeholder.priceUnitPurpose")}
                rows={2}
                className="text-sm resize-none"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea value={form.locale.usage_example}
                ...
                placeholder={t("placeholder.priceUnitUsageExample")}
                rows={2}
                className="text-sm resize-none"
              />
            </div>
