...
export function PriceUnitCard({ priceUnit, defaultName, onView, onDelete }: PriceUnitCardProps) {
  ...
  const subtitle = defaultName?.trim()
    ? `${priceUnit.code} · ID #${priceUnit.id}`
    : `ID #${priceUnit.id}`;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        >
          {onView && (
            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={(e) => { e.stopPropagation(); onView(priceUnit); }}>
          ...
          {onDelete && (
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); e.preventDefault(); onDelete(priceUnit); }}
            >
      ...
      <div className="mt-4 pt-3 border-t flex items-center justify-between">
        <Badge variant="secondary">#{priceUnit.sort_order}</Badge>
        <span className="text-xs text-muted-foreground">
          {priceUnit.locales.length} locale{priceUnit.locales.length !== 1 ? "s" : ""}
        </span>
      </div>
    </Card>
  );
