...
export interface PriceScopePickerDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Already-picked scope ids — excluded from the pickable list so the same scope can't be added twice. */
  excludeIds?: number[];
  onSelect: (scope: PriceScope) => void;
}

export function PriceScopePickerDialog({ open, onOpenChange, excludeIds, onSelect }: PriceScopePickerDialogProps) {
  const { t } = useTranslation();
  const [scopes, setScopes] = useState<PriceScope[]>([]);
  ...
  async function load(p: number, q: string) {
    setLoading(true);
    try {
      const res = await priceScopesService.list({
        page: p,
        size: PAGE_SIZE,
        sort_by: "code",
      ...
      toast.error(err instanceof Error ? err.message : t("priceScopePicker.loadFailed"));
    } finally {
      setLoading(false);
    }
  }

  function handleSelect(scope: PriceScope) {
    onSelect(scope);
    onOpenChange(false);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl p-0 gap-0 overflow-hidden flex flex-col max-h-[85vh]">
        <DialogTitle className="sr-only">{t("priceScopePicker.title")}</DialogTitle>
        ...
        <div className="flex flex-col gap-3 px-5 pt-4 pb-4 pr-12 border-b shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold">{t("priceScopePicker.title")}</p>
              {totalElements > 0 && (
                <p className="text-xs text-muted-foreground">{t("priceScopePicker.count", { n: totalElements })}</p>
              )}
            </div>
          </div>
          <div className="relative">
            ...
            <Input
              ...
              placeholder={t("priceScopePicker.searchPlaceholder")}
