...
export function CountryPickerDialog({ open, onOpenChange, selectedId, onSelect }: CountryPickerDialogProps) {
  ...
  async function load(p: number, q: string) {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t("countryPicker.loadFailed"));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl p-0 gap-0 overflow-hidden flex flex-col max-h-[85vh]">
        <DialogTitle className="sr-only">{t("countryPicker.title")}</DialogTitle>
        ...
        <div className="flex flex-col gap-3 px-5 pt-4 pb-4 pr-12 border-b shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold">{t("countryPicker.title")}</p>
              {totalElements > 0 && (
                <p className="text-xs text-muted-foreground">{t("countryPicker.count", { n: totalElements })}</p>
              )}
            </div>
          </div>
