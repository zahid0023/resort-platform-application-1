...
export function CityCard({ city, defaultName, onView, onDelete }: CityCardProps) {
  ...
  const subtitle = defaultName?.trim() && city.code
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        >
          {onView && (
            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={(e) => { e.stopPropagation(); onView(city); }}>
      ...
      {city.code && (
        <div className="grid grid-cols-1 gap-3 text-sm mt-4">
          <div>
            ...
            <p className="font-medium font-mono">{city.code}</p>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 pt-3 border-t flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {city.locales.length} locale{city.locales.length !== 1 ? "s" : ""}
        </span>
      </div>
    </Card>
  );
