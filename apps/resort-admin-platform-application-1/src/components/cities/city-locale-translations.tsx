...
export interface CityLocaleTranslationsProps {
   ...
   * `GET /cities/{id}/locales/count` — not `form.locales`, which only ever holds one page (size 10)
   * of the paginated sub-resource and can miss codes past that page. `null` until the first fetch
   * resolves, in which case `usedLocaleIds` falls back to `form.locales`. */
  cityLocaleCodes: string[] | null;
}
...
export function CityLocaleTranslations({
  ...
  function refreshLocales(localeCode?: string) {
    ...
    citiesService.listLocales(cityId, { size: 10, localeCode: localeCode || undefined })
      .then((res) => {
        onFormChange({
          ...form,
  ...
  function usedLocaleIds(excludeKey?: string): Set<number> {
    ...
    // Prefer the authoritative, unpaginated code list from the city's own /locales/count
    ...
    if (cityLocaleCodes) {
      ...
      const existing = cityLocaleCodes.map((c) => idByCode.get(c));
  ...
  // in the session) — before that, every code in `cityLocaleCodes` would resolve to `undefined` and
  // get silently dropped, undercounting used locales and leaving +Add wrongly enabled even when
  ...
  function usedLocaleCount(excludeKey?: string): number {
    ...
    if (cityLocaleCodes) return cityLocaleCodes.length + addedCount;
