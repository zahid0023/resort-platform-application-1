...
export interface CityDialogProps {
  ...
  mode: CityDialogMode;
  cityId?: number;
  /** Pre-set country (country detail page). When absent, a country selector is shown in create mode. */
  countryId?: number;
  form: CityFormState;
...
export function CityDialog({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const resolvedCountryId = countryId ?? (form.country_id !== "" ? Number(form.country_id) : undefined);
    if (!resolvedCountryId) { toast.error("Please select a country."); return; }
  ...
  const headerTitle = mode === "create"
    ? t("dialog.city.new")
    : (isEditing ? t("dialog.city.edit") : t("dialog.city.view"));
  const headerDesc = mode === "create"
    ? t("dialog.city.desc.create")
    : (isEditing ? t("dialog.city.desc.edit") : t("dialog.city.desc.view"));
