...
export function CityDialog({
  ...
  async function fetchLocales(localeCode?: string): Promise<void> {
    ...
    const res = await citiesService.listLocales(cityId, { size: 10, localeCode: localeCode || undefined });
    onFormChange({
      ...form,
  ...
  // unfiltered list (see CityLocaleTranslations' duplicate-locale checks).
  useEffect(() => {
  ...
  // existing translation doesn't need this: CityLocaleTranslations drives `translationsEditing`
  ...
  async function handleRefresh() {
    ...
    try {
      if (activeTab === "general") {
        const res = await citiesService.get(cityId);
        const full = res.data;
        onFormChange({
          ...form,
