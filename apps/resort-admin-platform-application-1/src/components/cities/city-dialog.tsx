...
import { Check, Languages, MapPin, Pencil, Plus, Trash2, X } from "lucide-react";
...
export function CityDialog({
  ...
  // Reset all editing state when dialog closes
  useEffect(() => {
    if (!open) {
  ...
  async function saveGeneral() {
    ...
    try {
      await citiesService.update(countryId, cityId, {
  ...
  async function saveRow(key: string, row: CityLocaleRow, isNew: boolean) {
    ...
    try {
      if (isNew) {
        await citiesService.addLocale(countryId, cityId, {
        ...
        await citiesService.updateLocale(countryId, cityId, row.id!, {
          name: data.name.trim(),
          description: data.description?.trim() || undefined,
          sort_order: Number(data.sort_order) || 0,
        });
  ...
  async function deleteRow(row: CityLocaleRow) {
    if (cityId == null || !row.id) return;
    const key = rowKey(row);
    setBusy(key, true);
    try {
      await citiesService.removeLocale(countryId, cityId, row.id);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-base font-semibold leading-tight">
                  ...
                  {mode !== "create" && (generalEditing || translationsEditing ? t("dialog.city.edit") : t("dialog.city.view"))}
                </DialogTitle>
                <DialogDescription className="text-xs mt-0.5">
                  {mode === "create" && t("dialog.city.desc.create")}
                  {mode !== "create" && (generalEditing || translationsEditing ? t("dialog.city.desc.edit") : t("dialog.city.desc.view"))}
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border bg-card overflow-hidden">
                ...
                {/* TRANSLATIONS EDITING mode — per-row editing */}
                {translationsEditing && (
